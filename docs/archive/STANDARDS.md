...
- If GitHub Actions shows `startup_failure` with `jobs: []`, report it as a workflow-level blocker and do not treat the branch as green until an actual job runs.
- Merge a PR only after linked PR gates complete successfully on the PR head.
