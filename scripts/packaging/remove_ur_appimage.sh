#!/bin/bash
# Remove pistisai-appimage from AUR
# This script should be run locally with AUR SSH access

set -e

echo "Removing pistisai-appimage from AUR..."
...
# Clone the AUR repo
git clone ssh://aur@aur.archlinux.org/pistisai-appimage.git aur-remove
cd aur-remove
...
git commit -m "Remove package - consolidated into pistisai" || echo "Nothing to commit"
