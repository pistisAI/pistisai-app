...
set -e
if [[ $status -eq 0 ]]; then
  echo "Expected get_helm.sh to fail when tar fails" >&2
...
if [[ -e "$created_path" ]]; then
  echo "Expected Helm temp root cleanup to remove $created_path after failure" >&2
  exit 1
fi
if [[ -e "$INSTALL_DIR/helm" ]]; then
  echo "Did not expect helm binary to be installed on tar failure" >&2
  exit 1
fi
