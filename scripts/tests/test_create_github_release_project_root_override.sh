...
LOG_FILE="/tmp/test_create_github_release_project_root_override.commands.log"
mkdir -p "$FAKE_ROOT/scripts/release" "$FAKE_ROOT/dist/windows" "$FAKE_ROOT/dist/linux" "$FAKE_BIN" "$WORK_DIR/tmp"
export LOG_FILE
rm -f "$LOG_FILE"
...
set +e
( cd "$FAKE_ROOT" && PROJECT_ROOT_OVERRIDE="$FAKE_ROOT" TMPDIR="$WORK_DIR/tmp" PATH="$FAKE_BIN:/usr/bin:/bin" GH_CMD="$FAKE_BIN/fake-gh" GIT_CMD="$FAKE_BIN/fake-git" bash scripts/release/create_github_release.sh ) > /tmp/test_create_github_release_project_root_override.log 2>&1
status=$?
set -e
...
if ! grep -Fq 'GitHub release created successfully!' /tmp/test_create_github_release_project_root_override.log; then
  echo "Missing success output" >&2
  cat /tmp/test_create_github_release_project_root_override.log >&2
  exit 1
fi

if ! grep -Fq 'Current version: 2.3.4' /tmp/test_create_github_release_project_root_override.log; then
...
echo "[test_create_github_release_project_root_override] Passed"
