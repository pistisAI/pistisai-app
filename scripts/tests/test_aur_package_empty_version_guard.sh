...
name: pistisai
version:
description: Empty version test
EOF

cat > "$BIN_DIR/flutter_with_cleanup.sh" <<'EOF'
#!/bin/bash
echo "flutter should not be called for empty version guard" >&2
exit 42
EOF
...
if PATH="$BIN_DIR:$PATH" FLUTTER_CMD="$BIN_DIR/flutter_with_cleanup.sh" PROJECT_ROOT_OVERRIDE="$WORK_DIR" bash "$WORK_DIR/test_aur_package.sh" > /dev/null 2>"$OUTPUT_FILE"; then
  echo "test_aur_package.sh unexpectedly succeeded with empty version entry" >&2
  exit 1
fi
...
echo "[test_aur_package_empty_version_guard] Passed"
