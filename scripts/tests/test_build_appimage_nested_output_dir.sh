...
TMP_OUTPUT="$TMP_OUTPUT_ROOT/nested/out/cloudtolocalllm-appimage-nested.AppImage"
TMP_DESKTOP_TEMPLATE="$(mktemp)"
TMP_INVOKE_LOG="$(mktemp /tmp/appimage-nested-output.XXXXXX.log)"
...
Comment=Nested output dir smoke test desktop entry
Terminal=false
EOF
...
"$PROJECT_ROOT/scripts/build-appimage.sh" >/tmp/test_build_appimage_nested_output_dir.log 2>&1

if [[ ! -f "$TMP_OUTPUT" ]]; then
  echo "Expected AppImage output at $TMP_OUTPUT" >&2
  cat /tmp/test_build_appimage_nested_output_dir.log >&2
  exit 1
fi

if [[ ! -f "$TMP_OUTPUT" ]]; then
  echo "Expected nested AppImage output file to be created" >&2
  cat /tmp/test_build_appimage_nested_output_dir.log >&2
  exit 1
fi
...
if grep -q 'Downloading appimagetool' /tmp/test_build_appimage_nested_output_dir.log; then
  echo "Expected APPIMAGETOOL_CMD override to skip download path" >&2
  cat /tmp/test_build_appimage_nested_output_dir.log >&2
  exit 1
fi

echo "[test_build_appimage_nested_output_dir] Passed"
