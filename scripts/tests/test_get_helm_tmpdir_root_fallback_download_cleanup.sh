...
#!/usr/bin/env bash
set -euo pipefail
echo "tar should not be invoked when curl fails" >&2
exit 1
EOF
chmod +x "$FAKE_BIN/tar"

set +e
TMPDIR="/" PATH="$INSTALL_DIR:$FAKE_BIN:$PATH" USE_SUDO=false VERIFY_CHECKSUM=false VERIFY_SIGNATURES=false DESIRED_VERSION='v1.2.3' HELM_INSTALL_DIR="$INSTALL_DIR" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -eq 0 ]]; then
  echo "Expected get_helm.sh to fail when curl fails" >&2
...
if [[ -e "$created_path" ]]; then
  echo "Expected Helm temp root cleanup to remove $created_path after curl failure" >&2
  exit 1
fi
if [[ -e "$INSTALL_DIR/helm" ]]; then
  echo "Did not expect helm binary to be installed on curl failure" >&2
  exit 1
fi
