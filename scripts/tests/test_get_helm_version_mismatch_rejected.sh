...
if [[ $status -eq 0 ]]; then
  ...
  echo "Expected get_helm.sh to reject a version mismatch" >&2
  exit 1
fi
if ! grep -q 'Helm v9.9.9 is installed in .*install, but expected v1.2.3.' "$FAKE_ROOT/output.log"; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected a clear version mismatch error" >&2
  exit 1
fi
