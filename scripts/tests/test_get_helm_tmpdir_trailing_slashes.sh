...
set +e
TMPDIR="/tmp////" PATH="$INSTALL_DIR:$FAKE_BIN:$PATH" USE_SUDO=false VERIFY_CHECKSUM=false VERIFY_SIGNATURES=false DESIRED_VERSION='v1.2.3' HELM_INSTALL_DIR="$INSTALL_DIR" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -ne 0 ]]; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected get_helm.sh to succeed with TMPDIR=/tmp////" >&2
  exit 1
fi
if ! grep -q '^template=/tmp/helm-installer\.' "$MK_TEMP_LOG"; then
  cat "$MK_TEMP_LOG" >&2
  echo "Expected mktemp to normalize trailing slashes to /tmp" >&2
  exit 1
fi
...
if [[ -e "$created_path" ]]; then
  echo "Expected Helm temp root cleanup to remove $created_path" >&2
  exit 1
fi
