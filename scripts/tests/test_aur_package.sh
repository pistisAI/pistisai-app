...
APPIMAGE_PATH="$DIST_DIR/Pistisai-Linux-${VERSION}-x86_64.AppImage"
...
sed -i "s|source=(.*)|source=(\"Pistisai-Linux-${VERSION}-x86_64.AppImage\")|" "$AUR_TEST_DIR/PKGBUILD"
sed -i "s/sha256sums=(.*)/sha256sums=('$CHECKSUM')/" "$AUR_TEST_DIR/PKGBUILD"

# Link the local AppImage so makepkg can find it
ln -sf "$APPIMAGE_PATH" "$AUR_TEST_DIR/Pistisai-Linux-${VERSION}-x86_64.AppImage"
