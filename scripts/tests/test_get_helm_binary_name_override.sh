...
set +e
TMPDIR="/tmp////" PATH="$FAKE_BIN" USE_SUDO=false VERIFY_CHECKSUM=true VERIFY_SIGNATURES=true DESIRED_VERSION='v1.2.3' BINARY_NAME='helm3' HELM_INSTALL_DIR="$INSTALL_DIR////" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -ne 0 ]]; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected get_helm.sh to succeed with BINARY_NAME override" >&2
  exit 1
fi
if ! grep -q '^Preparing to install helm3 into .*/install$' "$FAKE_ROOT/output.log"; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected install message to reflect the BINARY_NAME override" >&2
  exit 1
fi
if ! grep -q '^helm3 installed into .*/install/helm3$' "$FAKE_ROOT/output.log"; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected final install message to use the overridden binary name" >&2
  exit 1
fi
if [[ ! -x "$INSTALL_DIR/helm3" ]]; then
  echo "Expected overridden binary to be installed at $INSTALL_DIR/helm3" >&2
  exit 1
fi
