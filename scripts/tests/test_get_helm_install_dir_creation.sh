...
INSTALL_PARENT="$FAKE_ROOT/custom"
INSTALL_DIR="$INSTALL_PARENT/install"
mkdir -p "$FAKE_BIN" "$INSTALL_PARENT"
...
set +e
TMPDIR="/tmp////" PATH="$INSTALL_DIR:$FAKE_BIN" USE_SUDO=false VERIFY_CHECKSUM=true VERIFY_SIGNATURES=true DESIRED_VERSION='v1.2.3' HELM_INSTALL_DIR="$INSTALL_DIR////" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -ne 0 ]]; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected get_helm.sh to succeed with missing HELM_INSTALL_DIR parent creation" >&2
  exit 1
fi
if ! grep -q '^Preparing to install helm into .*/custom/install$' "$FAKE_ROOT/output.log"; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected install path to be normalized and created" >&2
  exit 1
fi
...
if [[ -e "$created_path" ]]; then
  echo "Expected Helm temp root cleanup to remove $created_path after success" >&2
  exit 1
fi
if [[ ! -x "$INSTALL_DIR/helm" ]]; then
  echo "Expected helm binary to be installed at $INSTALL_DIR/helm" >&2
  exit 1
fi
