...
OUTPUT_DEB="$FAKE_ROOT/nested/deep/out/cloudtolocalllm_2.3.4_amd64.deb"
...
bash "$PROJECT_ROOT/scripts/packaging/build_deb.sh" >/tmp/test_build_deb_nested_output_dir.log 2>&1

if [[ ! -f "$OUTPUT_DEB" ]]; then
  echo "Expected Debian output at $OUTPUT_DEB" >&2
  cat /tmp/test_build_deb_nested_output_dir.log >&2
  exit 1
fi
...
echo "[test_build_deb_nested_output_dir] Passed"
