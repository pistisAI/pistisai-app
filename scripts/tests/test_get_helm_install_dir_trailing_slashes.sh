...
if [[ $status -ne 0 ]]; then
  ...
  echo "Expected get_helm.sh to succeed with HELM_INSTALL_DIR=/tmp...////" >&2
  exit 1
fi
if ! grep -q '^Preparing to install helm into /tmp/.*/install$' "$FAKE_ROOT/output.log"; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected install path to be normalized without trailing slashes" >&2
  exit 1
fi
