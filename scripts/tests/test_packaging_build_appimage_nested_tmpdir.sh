...
"$TARGET_SCRIPT" >/tmp/test_packaging_build_appimage_nested_tmpdir.log 2>&1

if [[ ! -f "$TMP_OUTPUT" ]]; then
  echo "Expected AppImage output at $TMP_OUTPUT" >&2
  cat /tmp/test_packaging_build_appimage_nested_tmpdir.log >&2
  exit 1
fi

if ! grep -Fq "$TMPDIR_ROOT" "$APPIMAGE_LOG"; then
  echo "Expected build_appimage.sh to use nested TMPDIR for its workdir" >&2
  cat /tmp/test_packaging_build_appimage_nested_tmpdir.log >&2
  cat "$APPIMAGE_LOG" >&2
  exit 1
fi

echo "[test_packaging_build_appimage_nested_tmpdir] Passed"
