...
INSTALL_PARENT="$FAKE_ROOT/custom dir"
INSTALL_DIR="$INSTALL_PARENT/install dir"
...
set +e
TMPDIR="/tmp////" PATH="$FAKE_BIN" USE_SUDO=false VERIFY_CHECKSUM=true VERIFY_SIGNATURES=true DESIRED_VERSION='v1.2.3' HELM_INSTALL_DIR="$INSTALL_DIR////" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -ne 0 ]]; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected get_helm.sh to succeed with HELM_INSTALL_DIR containing spaces" >&2
  exit 1
fi
if ! grep -Fq "Preparing to install helm into $INSTALL_DIR" "$FAKE_ROOT/output.log"; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected install path with spaces to be preserved and normalized" >&2
  exit 1
fi
...
if [[ ! -x "$INSTALL_DIR/helm" ]]; then
  echo "Expected helm binary at $INSTALL_DIR/helm" >&2
  exit 1
fi
