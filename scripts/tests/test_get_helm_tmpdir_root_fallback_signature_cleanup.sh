...
#!/usr/bin/env bash
set -euo pipefail
if [[ "${1:-}" == "--batch" && "${2:-}" == "--quiet" && "${5:-}" == "--import" ]]; then
  echo "gpg import failure" >&2
  exit 1
fi
...
set +e
TMPDIR="/" PATH="$INSTALL_DIR:$FAKE_BIN:$PATH" USE_SUDO=false VERIFY_CHECKSUM=false VERIFY_SIGNATURES=true DESIRED_VERSION='v1.2.3' HELM_INSTALL_DIR="$INSTALL_DIR" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -eq 0 ]]; then
  echo "Expected get_helm.sh to fail when gpg import fails" >&2
  cat "$FAKE_ROOT/output.log" >&2
  exit 1
fi
if ! grep -q '^template=/tmp/helm-installer\.' "$MK_TEMP_LOG"; then
  cat "$MK_TEMP_LOG" >&2
  echo "Expected mktemp to normalize TMPDIR=/ to /tmp" >&2
  exit 1
fi
...
if [[ -e "$created_path" ]]; then
  echo "Expected Helm temp root cleanup to remove $created_path after signature failure" >&2
  exit 1
fi
if [[ -e "$INSTALL_DIR/helm" ]]; then
  echo "Did not expect helm binary to be installed on signature failure" >&2
  exit 1
fi
