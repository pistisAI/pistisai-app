...
if [[ $status -ne 0 ]]; then
  echo "Expected setup-development-environment.sh to succeed with trailing-slash TMPDIR" >&2
  cat "$WORKDIR/output.log" >&2
  exit 1
fi

if ! grep -Fq 'git:clone https://aur.archlinux.org/yay.git' "$CALLS_LOG"; then
  echo "Expected yay clone to run" >&2
  cat "$CALLS_LOG" >&2
  exit 1
fi
...
if ! grep -Fq 'makepkg-stub -si --noconfirm' "$CALLS_LOG"; then
  echo "Expected makepkg stub to run inside the yay checkout" >&2
  cat "$CALLS_LOG" >&2
  exit 1
fi
...
if find "$TMPDIR_EXPECTED" -maxdepth 1 -type d -name 'yay-install.*' | grep -q .; then
  echo "Expected yay build directory cleanup under normalized TMPDIR root" >&2
...
echo "[test_setup_development_environment_tmpdir_trailing_slashes] Passed"
