...
if [[ ! -f "$BASHRC_FILE" ]]; then
  echo "Expected custom BASHRC_FILE with spaces to be created" >&2
  exit 1
fi

path_count=$(grep -Fxc "export PATH=\"\$PATH:$WORKDIR/flutter-install/bin\"" "$BASHRC_FILE")
chrome_count=$(grep -Fxc 'export CHROME_EXECUTABLE=/usr/bin/chromium' "$BASHRC_FILE")

if [[ "$path_count" -ne 1 ]]; then
  echo "Expected exactly one Flutter PATH export line in spaced bashrc, found $path_count" >&2
  cat "$BASHRC_FILE" >&2
  exit 1
fi

if [[ "$chrome_count" -ne 1 ]]; then
  echo "Expected exactly one CHROME_EXECUTABLE export line in spaced bashrc, found $chrome_count" >&2
  cat "$BASHRC_FILE" >&2
  exit 1
fi
...
echo "[test_setup_development_environment_bashrc_path_spaces] Passed"
