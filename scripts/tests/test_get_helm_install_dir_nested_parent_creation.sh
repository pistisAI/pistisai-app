...
INSTALL_DIR="$FAKE_ROOT/nested/inner/install"
mkdir -p "$FAKE_BIN"
...
if [[ $status -ne 0 ]]; then
  ...
  echo "Expected get_helm.sh to succeed with a nested HELM_INSTALL_DIR parent" >&2
  exit 1
fi
if ! grep -q '^Preparing to install helm into .*/nested/inner/install$' "$FAKE_ROOT/output.log"; then
  cat "$FAKE_ROOT/output.log" >&2
  echo "Expected nested install path to be normalized and created" >&2
  exit 1
fi
