...
if ! grep -Fq 'config.sh --url https://github.com/pistisAI/pistisai-app --token RUNNER_TOKEN_1' "$RUNNER_DIR/config.log"; then
  echo "Expected first configuration run to record the runner token" >&2
  cat "$RUNNER_DIR/config.log" >&2
  exit 1
fi

if ! grep -Fq 'config.sh --url https://github.com/pistisAI/pistisai-app --token RUNNER_TOKEN_2' "$RUNNER_DIR/config.log"; then
