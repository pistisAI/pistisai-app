...
cat > "$BIN_DIR/mv" <<EOF
#!/bin/bash
if [[ "\$1" == "$STAGING_DIR" && "\$2" == "$OUTPUT_DIR" ]]; then
  exit 1
fi
exec /bin/mv "\$@"
EOF
...
if [[ $status -eq 0 ]]; then
  echo "update_aur_pkgbuild.sh unexpectedly succeeded in restore harness" >&2
  cat "$WORK_DIR/output.log" >&2
  exit 1
fi

if [[ ! -f "$OUTPUT_DIR/original.txt" ]]; then
  echo "Expected original AUR output to be restored after publish failure" >&2
  ls -R "$WORK_DIR/dist" >&2
  cat "$WORK_DIR/output.log" >&2
  exit 1
fi

if ! grep -Fq 'keep-me' "$OUTPUT_DIR/original.txt"; then
  echo "Restored AUR output was not preserved correctly" >&2
  cat "$WORK_DIR/output.log" >&2
  exit 1
fi

if [[ -d "$BACKUP_DIR" ]]; then
  echo "Expected backup directory to be cleaned up after restore" >&2
  ls -ld "$BACKUP_DIR" >&2
  exit 1
fi

echo "[test_update_aur_pkgbuild_restore] Passed"
