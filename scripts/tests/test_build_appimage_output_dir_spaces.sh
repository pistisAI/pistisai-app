...
"$PROJECT_ROOT/scripts/build-appimage.sh" >/tmp/test_build_appimage_output_dir_spaces.log 2>&1

if [[ ! -f "$TMP_OUTPUT" ]]; then
  echo "Expected AppImage output at $TMP_OUTPUT" >&2
  cat /tmp/test_build_appimage_output_dir_spaces.log >&2
  exit 1
fi
...
if grep -q 'Downloading appimagetool' /tmp/test_build_appimage_output_dir_spaces.log; then
  echo "Expected APPIMAGETOOL_CMD override to skip download path" >&2
  cat /tmp/test_build_appimage_output_dir_spaces.log >&2
  exit 1
fi

echo "[test_build_appimage_output_dir_spaces] Passed"
