...
if [[ $status -ne 0 ]]; then
  echo "Expected setup-development-environment.sh to succeed with TMPDIR=/" >&2
  cat "$WORKDIR/output.log" >&2
  exit 1
fi

if ! grep -Fq "git:clone https://aur.archlinux.org/yay.git $TMPDIR_EXPECTED/yay-install." "$CALLS_LOG"; then
  echo "Expected yay build dir to fall back to /tmp" >&2
  cat "$CALLS_LOG" >&2
  exit 1
fi
...
if [[ -d "$build_dir" ]]; then
  echo "Expected yay build directory cleanup for TMPDIR=/" >&2
  printf '%s\n' "$build_dir" >&2
  exit 1
fi
...
echo "[test_setup_development_environment_tmpdir_root_fallback] Passed"
