...
TMPDIR_OVERRIDE="/"
LOG_FILE="/tmp/test_create_github_release_tmpdir_root_fallback.commands.log"
...
set +e
( cd "$FAKE_ROOT" && PROJECT_ROOT_OVERRIDE="$FAKE_ROOT" TMPDIR="$TMPDIR_OVERRIDE" PATH="$FAKE_BIN:/usr/bin:/bin" GH_CMD="$FAKE_BIN/fake-gh" GIT_CMD="$FAKE_BIN/fake-git" bash scripts/release/create_github_release.sh ) > /tmp/test_create_github_release_tmpdir_root_fallback.log 2>&1
status=$?
set -e

if [[ $status -ne 0 ]]; then
  echo "Expected create_github_release.sh to succeed with TMPDIR=/ fallback" >&2
  cat /tmp/test_create_github_release_tmpdir_root_fallback.log >&2
  exit 1
fi

if ! grep -Fq 'GitHub release created successfully!' /tmp/test_create_github_release_tmpdir_root_fallback.log; then
...
echo "[test_create_github_release_tmpdir_root_fallback] Passed"
