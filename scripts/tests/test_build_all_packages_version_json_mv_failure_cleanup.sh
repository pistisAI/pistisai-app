...
"$TARGET_SCRIPT" --packages appimage >/tmp/test_build_all_packages_version_json_mv_failure_cleanup.log 2>&1
status=$?
set -e

if [[ $status -eq 0 ]]; then
  echo "Expected build_all_packages.sh to fail when version.json mv fails" >&2
  cat /tmp/test_build_all_packages_version_json_mv_failure_cleanup.log >&2
  exit 1
fi

if ! grep -Fxq "  static const String appVersion = 'old';" "$FAKE_ROOT/lib/config/app_config.dart"; then
  echo "Expected app_config.dart to be restored after version.json mv failure" >&2
  cat /tmp/test_build_all_packages_version_json_mv_failure_cleanup.log >&2
...
if ! grep -Fxq '{"version":"old","build_number":"0"}' "$FAKE_ROOT/assets/version.json"; then
  echo "Expected version.json to be restored after mv failure" >&2
  cat /tmp/test_build_all_packages_version_json_mv_failure_cleanup.log >&2
...
if compgen -G "$FAKE_ROOT/assets/.version.json.*" > /dev/null; then
  echo "Expected version.json temp file cleanup after mv failure" >&2
  ls -1 "$FAKE_ROOT/assets" >&2
  exit 1
fi

if ! grep -Fq 'assets/version.json' "$LOG_FILE"; then
  echo "Expected mv failure path to be exercised for version.json" >&2
  cat "$LOG_FILE" >&2
  exit 1
fi

echo "[test_build_all_packages_version_json_mv_failure_cleanup] Passed"
