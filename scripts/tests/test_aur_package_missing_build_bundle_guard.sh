...
mkdir -p "$WORK_DIR/scripts" "$WORK_DIR/build-tools/packaging/aur"
...
if PROJECT_ROOT_OVERRIDE="$WORK_DIR" FLUTTER_CMD="$WORK_DIR/flutter_with_cleanup.sh" MAKEPKG_CMD="$WORK_DIR/makepkg-wrapper" bash "$WORK_DIR/test_aur_package.sh" > /dev/null 2>"$OUTPUT_FILE"; then
  echo "test_aur_package.sh unexpectedly succeeded with missing build bundle" >&2
  exit 1
fi

if ! grep -q "Flutter Linux build bundle not found" "$OUTPUT_FILE"; then
  echo "Expected missing build bundle error message" >&2
  cat "$OUTPUT_FILE" >&2
  exit 1
fi

echo "[test_aur_package_missing_build_bundle_guard] Passed"
