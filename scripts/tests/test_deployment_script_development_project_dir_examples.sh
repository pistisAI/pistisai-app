...
for needle in \
  'cd \${PROJECT_DIR:-/opt/Pistisai} && ./scripts/deploy/complete_deployment.sh --force' \
...
if grep -Fq 'cd /opt/Pistisai && ./scripts/deploy/complete_deployment.sh --force' "$FILE"; then
  echo "found legacy hardcoded VPS deployment example (PowerShell comment)" >&2
  exit 1
fi

if grep -Fq "cd /opt/Pistisai && bash scripts/deploy/complete_deployment.sh" "$FILE"; then
