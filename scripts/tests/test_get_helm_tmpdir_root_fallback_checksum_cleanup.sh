...
#!/usr/bin/env bash
set -euo pipefail
echo "SHA256(/tmp/fake) = cafe1234"
EOF
chmod +x "$FAKE_BIN/openssl"

set +e
TMPDIR="/" PATH="$INSTALL_DIR:$FAKE_BIN:$PATH" USE_SUDO=false VERIFY_SIGNATURES=false DESIRED_VERSION='v1.2.3' HELM_INSTALL_DIR="$INSTALL_DIR" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -eq 0 ]]; then
  echo "Expected get_helm.sh to fail on checksum mismatch" >&2
...
if [[ -e "$created_path" ]]; then
  echo "Expected Helm temp root cleanup to remove $created_path after checksum failure" >&2
  exit 1
fi
if [[ -e "$INSTALL_DIR/helm" ]]; then
  echo "Did not expect helm binary to be installed on checksum failure" >&2
  exit 1
fi
