...
set +e
TMPDIR="/" PATH="$FAKE_BIN:$PATH" USE_SUDO=false VERIFY_CHECKSUM=false VERIFY_SIGNATURES=false DESIRED_VERSION='v1.2.3' HELM_INSTALL_DIR="$INSTALL_DIR" bash "$TARGET_SCRIPT" > "$FAKE_ROOT/output.log" 2>&1
status=$?
set -e
if [[ $status -eq 0 ]]; then
