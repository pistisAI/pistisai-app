...
for needle in \
  '# Creates all release assets: `pistisai-<version>-portable.zip`, `Pistisai-Windows-<version>-Setup.exe`, and checksums' \
  'Write-Host "  • pistisai-$Version-portable.zip"' \
  'Write-Host "  • Pistisai-Windows-$Version-Setup.exe"' \
  'Write-LogInfo "Creating pistisai-$Version-portable.zip..."' \
  'Write-LogSuccess "pistisai-$Version-portable.zip created successfully: $packageName"' \
  ...
  '# Create pistisai-$Version-portable.zip' \
  '# Create Pistisai-Windows-$Version-Setup.exe if not skipped' \
  'Write-LogInfo "pistisai-$Version-portable.zip location: $zipPath"' \
