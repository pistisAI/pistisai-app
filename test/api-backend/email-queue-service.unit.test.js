...
describe("EmailQueueService", () => {
  describe("constructor", () => {
    ...
    it("should store provided dependencies", () => {
      const { service, db, googleWorkspaceService, emailConfigService } =
        createService();
  ...
  describe("_calculateBackoffDelay", () => {
    ...
    it("should return INITIAL_RETRY_DELAY for retryCount=1", () => {
      expect(service._calculateBackoffDelay(1)).toBe(
        service.INITIAL_RETRY_DELAY,
      );
    });
  });
  ...
  describe("_checkRateLimit / _updateRateLimit", () => {
    ...
    it("should track and enforce user rate limit", () => {
      ...
      expect(() => service._checkRateLimit("user-1")).toThrow(
    ...
    it("should enforce system rate limit", () => {
      ...
      expect(() => service._checkRateLimit("any-user")).toThrow(
  ...
  describe("queueEmail", () => {
    ...
    it("should render template when templateName provided", async () => {
      ...
      emailConfigService.getTemplate.mockResolvedValue({
        html: "tmpl-html",
        text: "tmpl-text",
      });
      ...
      expect(emailConfigService.getTemplate).toHaveBeenCalledWith(
        "welcome",
        "user-1",
      );
  ...
  describe("processPendingEmails", () => {
    ...
    it("should process pending emails successfully", async () => {
      const { service, db, emailConfigService, googleWorkspaceService } =
        createService();
      ...
      emailConfigService.getConfiguration.mockResolvedValue({
        is_active: true,
        from_address: "from@example.com",
        google_oauth_token_encrypted: "token",
      });
  ...
  describe("_handleEmailFailure", () => {
    ...
    it("should handle errors in failure handler gracefully", async () => {
      ...
      // Should not throw
      await expect(
        service._handleEmailFailure(email, new Error("Send failed")),
  ...
  describe("_sendEmail SMTP fallback", () => {
    ...
    it("should throw when no active config found", async () => {
      ...
      await expect(service._sendEmail(email)).rejects.toThrow(
        /No active email configuration/,
      );
    });
  });
});
