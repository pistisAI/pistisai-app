...
describe("ErrorNotificationService", () => {
  ...
  describe("Constructor", () => {
    test("should initialize with default config", () => {
      ...
      expect(svc.config.notificationChannels).toEqual([
    ...
    test("should register default LOG handler", () => {
      expect(service.notificationHandlers.has(NotificationChannel.LOG)).toBe(
        true,
      );
    });
    ...
    test("should register WEBHOOK handler when webhookUrl configured", () => {
      ...
      expect(svc.notificationHandlers.has(NotificationChannel.WEBHOOK)).toBe(
        true,
      );
    });
    ...
    test("should not register WEBHOOK handler when no webhookUrl", () => {
      expect(
        service.notificationHandlers.has(NotificationChannel.WEBHOOK),
      ).toBe(false);
    });
    ...
    test("should register EMAIL handler when emailService configured", () => {
      ...
      expect(svc.notificationHandlers.has(NotificationChannel.EMAIL)).toBe(
        true,
      );
    });
    ...
    test("should register SLACK handler when slackWebhook configured", () => {
      ...
      expect(svc.notificationHandlers.has(NotificationChannel.SLACK)).toBe(
        true,
      );
    });
  ...
  describe("_categorizeError", () => {
    test("should categorize database errors", () => {
      expect(
        service._categorizeError(new Error("database connection failed")),
    ...
    test("should categorize authentication errors", () => {
      expect(service._categorizeError(new Error("auth token expired"))).toBe(
        ErrorCategory.AUTHENTICATION,
      );
    });

    test("should categorize service errors", () => {
      expect(service._categorizeError(new Error("service unavailable"))).toBe(
        ErrorCategory.SERVICE,
      );
    });

    test("should categorize external API errors", () => {
      expect(service._categorizeError(new Error("fetch timeout"))).toBe(
        ErrorCategory.EXTERNAL_API,
      );
      expect(service._categorizeError(new Error("http 503"))).toBe(
        ErrorCategory.EXTERNAL_API,
      );
      expect(service._categorizeError(new Error("api rate limited"))).toBe(
        ErrorCategory.EXTERNAL_API,
      );
    });

    test("should categorize resource errors", () => {
      expect(service._categorizeError(new Error("memory exceeded"))).toBe(
        ErrorCategory.RESOURCE,
      );
      expect(service._categorizeError(new Error("resource exhausted"))).toBe(
        ErrorCategory.RESOURCE,
      );
    });

    test("should categorize system errors", () => {
      expect(service._categorizeError(new Error("system crash"))).toBe(
    ...
    test("should return UNKNOWN for unrecognized errors", () => {
      expect(
        service._categorizeError(new Error("something weird happened")),
  ...
  describe("_determineSeverity", () => {
    test("should mark DATABASE category as CRITICAL", () => {
      expect(
        service._determineSeverity(new Error("x"), ErrorCategory.DATABASE),
      ).toBe(ErrorSeverity.CRITICAL);
    });

    test("should mark SYSTEM category as CRITICAL", () => {
      expect(
        service._determineSeverity(new Error("x"), ErrorCategory.SYSTEM),
      ).toBe(ErrorSeverity.CRITICAL);
    });

    test("should detect critical in message", () => {
      expect(
        service._determineSeverity(
    ...
    test("should detect error keyword as HIGH", () => {
      expect(
        service._determineSeverity(
          new Error("error reading config"),
    ...
    test("should detect warning keyword as MEDIUM", () => {
      expect(
        service._determineSeverity(
          new Error("warning threshold reached"),
          ErrorCategory.UNKNOWN,
        ),
    ...
    test("should default SERVICE to HIGH", () => {
      expect(
        service._determineSeverity(new Error("x"), ErrorCategory.SERVICE),
      ).toBe(ErrorSeverity.HIGH);
    });

    test("should default unknown category to MEDIUM", () => {
      expect(
        service._determineSeverity(new Error("x"), ErrorCategory.UNKNOWN),
      ).toBe(ErrorSeverity.MEDIUM);
    });
  });
  ...
  describe("_shouldSendNotification", () => {
    test("should return false when notifications disabled", () => {
      ...
      expect(
        service._shouldSendNotification(
  ...
  describe("registerNotificationHandler", () => {
    ...
    test("should throw for non-function handler", () => {
      expect(() =>
        service.registerNotificationHandler("custom", "not-a-function"),
  ...
  describe("getErrorHistory", () => {
    ...
    test("should filter by severity", () => {
      ...
      expect(history.every((h) => h.severity === ErrorSeverity.CRITICAL)).toBe(
        true,
      );
    });
